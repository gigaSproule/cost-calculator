...
pub(crate) fn materials() -> gtk4::Grid {
    ...
    add_material_costs.connect_clicked(clone!(@strong material_costs_container, @strong material_costs_container_rows =>
        move |_| {
            add_material_row(&material_costs_container_rows, &material_costs_container, &material_costs_entries_add, StoredMaterial::default());
        }
    ));
...
fn add_material_row(
    material_costs_container_rows: &Rc<RefCell<i32>>,
    material_costs_container: &gtk4::Grid,
    ...
    material_remove.connect_clicked(clone!(@strong material_costs_container =>
    move |_| {
        ...
        material_costs_container.remove_row((row+1).try_into().unwrap());
    }));
    material_costs_container.attach(&material_remove, 3, *rows, 1, 1);
